...
from api.auth.helper import hash_password, verify_password, create_access_token,get_client_ip,decode_token_or_401, create_refresh_token,hash_refresh_token,validate_refresh_token
...
def login(response:Response,request:Request, form_data:Annotated[OAuth2PasswordRequestForm,Depends()], db:Annotated[Session, Depends(get_db)]):
    ...
    refresh_token,jti,exp= create_refresh_token({"sub": user_exist.id, "family_id": str(family_id)})
...
def refresh_token(response:Response, request:Request, db:Annotated[Session, Depends(get_db)], refresh_token:Annotated[Optional[str], Cookie(alias=settings.COOKIE_NAME)] = None):
    ...
    new_refresh_token,jti,exp = create_refresh_token({"sub": user_id, "family_id": str(token_exist.family_id)})
...
def logout(response:Response, db:Annotated[Session,Depends(get_db)], refresh_token:Annotated[Optional[str], Cookie(alias=settings.COOKIE_NAME)] = None):
    ...
    db.execute(update(Token).where(Token.family_id==family_id).values(revoked=True, revoked_at=datetime.utcnow()))
    db.commit()
